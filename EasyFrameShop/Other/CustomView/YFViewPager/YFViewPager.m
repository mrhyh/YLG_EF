...
- (void)drawRect:(CGRect)rect
{
    ...
    _bottomLineY =_pageControl.frame.size.height-_bottomLineToBottomSpace;
    _bottomLineW = (_pageControl.frame.size.width/_pageNum)/3*2;
    UILabel *selectedLabel = [[UILabel alloc] initWithFrame:CGRectMake(0 ,_bottomLineY, _bottomLineW, 2)];
...
- (void)setSelectIndex:(NSInteger)index
{
    ...
    if (_showAnimation) {
        [UIView animateWithDuration:0.3 animations:^{
            ...
            frame.origin.x = button.frame.origin.x + (button.frame.size.width-_bottomLineW)/2;
...
- (void) setBottomLineToBottomSpace:(CGFloat)bottomLineToBottomSpace {
    _bottomLineToBottomSpace = bottomLineToBottomSpace;
    if(_bottomLineToBottomSpace > _pageControl.frame.size.height) {
        _bottomLineToBottomSpace =  _pageControl.frame.size.height/2;
    }
     _bottomLineY =_pageControl.frame.size.height-_bottomLineToBottomSpace;
}
