...
@interface YJSegmentedControl ()<YJSegmentedControlDelegate>{
    CGFloat witdthFloat;
    CGFloat buttonLineWidthFloat; //按钮下划线宽度
...
- (void)AddSegumentArray:(NSArray *)SegumentArray{
    ...
    buttonLineWidthFloat = witdthFloat/5*3;
    ...
    for (int i = 0; i < SegumentArray.count; i++) {
        ...
        if (i == 0) {
            buttonDownView =[[UIView alloc]initWithFrame:CGRectMake((i * witdthFloat)+(witdthFloat-buttonLineWidthFloat)/2, self.bounds.size.height - (7+2), buttonLineWidthFloat, 2)];
...
-(void)selectTheSegument:(NSInteger)segument{
    ...
    if (selectSeugment != segument) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [buttonDownView setFrame:CGRectMake((segument * witdthFloat)+(witdthFloat-buttonLineWidthFloat)/2, self.bounds.size.height - (7+2), buttonLineWidthFloat, 2)];
        }];
