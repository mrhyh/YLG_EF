...
@interface DCPaymentView ()<UITextFieldDelegate>
{
    ...
    EFMailcompleteHandle completeHandle;
}
...
- (void)drawView {
    if (!_paymentAlert) {
        _paymentAlert = [[UIView alloc]initWithFrame:CGRectMake(40, [UIScreen mainScreen].bounds.size.height-KEYBOARD_HEIGHT-KEY_VIEW_DISTANCE * 0.3-ALERT_HEIGHT, [UIScreen mainScreen].bounds.size.width-80, ALERT_HEIGHT)];
...
- (void)EFMailcompleteHandle:(EFMailcompleteHandle)block {
    completeHandle = block;
}
